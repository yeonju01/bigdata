...
SELECT ENAME,HIREDATE, TRUNC(ADD_MONTHS(HIREDATE, 6), 'DAY')+1 FROM EMP;
--4. 이름, 입사일, 입사일로부터 현재까지의 개월수, 급여, 입사일부터 현재까지의 받은 급여의 총계를 출력
SELECT ENAME,HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) MONTH, SAL, SAL*TRUNC(MONTHS_BETWEEN(SYSDATE, HIREDATE)) FROM EMP;
...
SELECT EMPNO, ENAME, JOB, TO_CHAR(SAL, '$999,999,999') SAL FROM EMP WHERE DEPTNO = 20;
